...
4. Figures live in a sibling `images/` folder. `read` every main figure before writing the evidence chain. If a file is missing, mark that section `Not assessable from supplied material`.
...
- local Paper Matrix Markdown plus sibling `images/` (default);
...
Prefer the local Markdown and `images/` files. Prefer an existing `nature-reader` artifact when supplied. Do not repeat full bilingual translation or PDF figure extraction. If a linked figure file is missing, create a visibly partial card and mark that section `Not assessable from supplied material`.
