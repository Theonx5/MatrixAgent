...
- first list and `read` the paper's sibling `images/` directory (MinerU sync). Those files are the default assets,
- copy selected files into `notes/` or `reviews/` output (`output/assets/figures/`); never edit the synced `images/` folder,
- extract from a PDF only if the local `images/` file is missing and the user provided a PDF attachment,
...
- record source path, figure number, panel, crop status, and intended slide in `output/asset_manifest.md`.
