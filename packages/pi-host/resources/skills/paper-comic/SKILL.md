...
4. Figures live in a sibling `images/` folder. `read` the original method/result figures before drawing. If a file is missing, say it was not synced.
...
```bash
/paper-comic Attention is all you need
/paper-comic LLM/2017 - Attention.md --style sketchnote
/paper-comic 10.1/x --style paper-figure --language English --pages 1
```

默认从本地库 Markdown + 同级 `images/` 读论文和图。不要先去下 PDF。输出写到 `notes/` 或 `reviews/`。
