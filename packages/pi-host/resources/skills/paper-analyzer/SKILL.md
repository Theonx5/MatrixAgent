...
4. Figures live in a sibling `images/` folder. `read` every figure you will discuss or embed. If a file is missing, say it was not synced.
...
| 输入 | 执行 |
|------|------|
| 本地库题名/DOI/路径 | 从 `.sync/catalog.md` 定位，读取已同步的 Markdown。这是默认路径。 |
| 已有 Markdown | 直接读全文 |
| 附件 PDF/DOCX | 用 `read_attachment` 分页读取 |
| 粘贴文本 | 全部使用 |
| 不在库中的 arXiv/URL | 说明论文不在本地库；不要去抓出版社页面，除非用户明确要求 |

**自检**：有没有完整内容？没有 → 换方式继续，仍不要编造。
...
1. 列出论文 Markdown 里的 `images/` 链接，并 `read` 对应的本地图片（论文同级 `images/` 目录）。缺图就标明未同步，不要凭空描述。
2. 核心创新：论文做了什么别人没做的？（1-3个，每个一句话提炼）
3. 方法细节：输入→处理→输出→为什么更好（对着方法图讲）
4. 关键实验：哪个结果最有说服力？把对应结果图读进来。
5. 论文弱点：作者自述 + 你的判断
6. 代码对应：每个 component 对应哪个文件/函数
